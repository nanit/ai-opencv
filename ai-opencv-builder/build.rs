...
fn download_sources(source_root: &Path) -> Result<()> {
    for archive in ["opencv", "opencv_contrib"] {
        ...
        let status = Command::new("wget")
            ...
            .arg(&zip_path)
            .arg(format!(
                "https://github.com/opencv/{archive}/archive/{}.zip", opencv_version()
            ))
            .status()?;
...
fn main() -> Result<()> {
    ...
    println!("cargo::rustc-env=OPENCV_DIR={}", out_dir.join("install").join("lib").join("cmake").join("opencv4").to_str().unwrap());
...
fn opencv_version() -> String {
    ...
    version.split("opencv").last().expect(format!("version string: {version} is missing opencv version").as_str()).to_string()
}
